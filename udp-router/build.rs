...
/// This crate has a runtime dependency on artifacts produced by the `udp-router-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `udp-router-ebpf` would cause build (and
...
/// performance; an environment variable is used to replace true builds of `udp-router-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `udp-router-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "udp-router-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `udp-router-ebpf`, so cargo will automatically rebuild us
        // if `udp-router-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `udp-router-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("udp-router-ebpf");
